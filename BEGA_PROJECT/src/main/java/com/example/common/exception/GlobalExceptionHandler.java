...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse> handleResponseStatusException(ResponseStatusException ex) {
        ...
        if (message == null || message.isBlank()) {
            ...
            message = resolved != null ? resolved.getReasonPhrase() : "요청을 처리할 수 없습니다.";
        }

        if (statusCode.is5xxServerError()) {
            log.error("ResponseStatusException status={} message={}", statusCode.value(), message, ex);
        } else {
            log.warn("ResponseStatusException status={} message={}", statusCode.value(), message);
        }
